...
namespace BlazorDapperSPA.Data
{
    public class CityService : ICityService
    {
        ...
        public  IEnumerable<City> GetCitybySearch(string Name)
        {
             IEnumerable<City> cities;

            using (var conn = new SqlConnection(_configuration.Value))
            {
                const string query = @"select * from dbo.City where Name like '@Name%' ";

                if (conn.State == ConnectionState.Closed)
                    conn.Open();
                try
                {
                    cities =  conn.Query<City>(query, new { Name }, commandType: CommandType.Text);
                }
                catch (Exception ex)
                {
                    throw ex;
